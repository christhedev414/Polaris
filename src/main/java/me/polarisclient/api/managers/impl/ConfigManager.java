...
import me.polarisclient.mod.modules.settings.Bind;
import me.polarisclient.mod.modules.settings.EnumConverter;
...
public class ConfigManager implements Wrapper {
   ...
   public String config = "Polaris/config/";
   ...
   public static void setValueFromJson(Mod mod, Setting setting, JsonElement element) {
      ...
      switch(var4) {
         ...
         case "Color":
            ...
            return;
         default:
            Polaris.LOGGER.error("Unknown Setting type for: " + mod.getName() + " : " + setting.getName());
      }
   }
   ...
   public void loadConfig(String name) {
      List<File> files = Arrays.stream(Objects.requireNonNull(new File("Polaris").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
      if (files.contains(new File("Polaris/" + name + "/"))) {
         this.config = "Polaris/" + name + "/";
      } else {
         this.config = "Polaris/config/";
      }
   ...
   public boolean configExists(String name) {
      List<File> files = Arrays.stream(Objects.requireNonNull(new File("Polaris").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
      return files.contains(new File("Polaris/" + name + "/"));
   }
   ...
   public void saveCurrentConfig() {
      File currentConfig = new File("Polaris/currentconfig.txt");

      try {
         if (currentConfig.exists()) {
            FileWriter writer = new FileWriter(currentConfig);
            String tempConfig = this.config.replaceAll("/", "");
            writer.write(tempConfig.replaceAll("Polaris", ""));
            writer.close();
         } else {
   ...
   public void init() {
      ...
      Polaris.LOGGER.info("Config loaded.");
   }
   ...
   private void loadPath(Path path, Mod mod) throws IOException {
      ...
      try {
         ...
         Polaris.LOGGER.error("Bad Config File for: " + mod.getName() + ". Resetting...");
