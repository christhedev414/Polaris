...
package me.polarisclient.api.util;
...
public class PositionUtil implements Wrapper {
   ...
   public static Entity getPositionEntity() {
      ...
      return (Entity)(player == null
         ? null
         : ((ridingEntity = player.getRidingEntity()) != null && !(ridingEntity instanceof EntityBoat) ? ridingEntity : player));
   }
   ...
   public static boolean isAbove(BlockPos pos) {
      return mc.player.getEntityBoundingBox().minY >= (double)pos.getY();
   }

   public static BlockPos fromBB(AxisAlignedBB bb) {
      return new BlockPos(
         (bb.minX + bb.maxX) / 2.0, (bb.minY + bb.maxY) / 2.0, (bb.minZ + bb.maxZ) / 2.0
      );
   }

   public static boolean intersects(AxisAlignedBB bb, BlockPos pos) {
      return bb.intersects(
         (double)pos.getX(),
         (double)pos.getY(),
         (double)pos.getZ(),
         (double)(pos.getX() + 1),
