...
public class Desktop extends Module {
   ...
   final TrayIcon icon = new TrayIcon(this.image, "Polaris");
         ...
         try {
            for(Entity entity : this.players) {
               if (entity instanceof EntityPlayer
                  ...
                  this.icon.displayMessage("Polaris", entity.getName() + " has entered your visual range!", MessageType.INFO);
               }
            }
   ...
   public void onTotemPop(EntityPlayer player) {
      if (!fullNullCheck() && player == mc.player && this.selfPop.getValue()) {
         this.icon.displayMessage("Polaris", "You are popping!", MessageType.WARNING);
      }
   }
   ...
   public void onClientChatReceived(ClientChatReceivedEvent event) {
      if (!fullNullCheck()) {
         ...
         if (message.contains(mc.player.getName()) && this.mention.getValue()) {
            this.icon.displayMessage("Polaris", "New chat mention!", MessageType.INFO);
         }

         if (message.contains("whispers:") && this.dm.getValue()) {
            this.icon.displayMessage("Polaris", "New direct message!", MessageType.INFO);
         }
      }
   }
   ...
   private void addIcon() {
      ...
      this.icon.setToolTip("polaris alpha");
